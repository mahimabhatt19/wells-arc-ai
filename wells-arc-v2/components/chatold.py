...
def render_resolution_options():
    ...
    with col1:
        if st.button("📞 Connect with agent", use_container_width=True, key="connect_agent"):
            st.success("✅ Connecting you with a Wells Fargo agent. Average wait: 2 minutes.")
    with col2:
        if st.button("📅 Schedule callback", use_container_width=True, key="schedule_cb"):
            st.info("📅 A Wells Fargo agent will call you back within 2 hours.")
    with col3:
        if st.button("📄 Get PDF guide", use_container_width=True, key="get_pdf"):
